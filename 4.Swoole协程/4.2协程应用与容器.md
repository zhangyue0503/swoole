...
另外一定要先注册哦，如果在异常之后注册，那么肯定是没效果的啦，毕竟函数都还没有注册就出问题了，那就只能外部去处理异常了。
...
上面两种方式都可以并发地执行协程，我们都是使用原生的 sleep() ，如果是默认情况下，第一个测试需要等待 7 秒，第二个测试需要等待 6 秒，但是在并发的情况下，多个协程是一块执行的，所以都应该是以最大的等待时间为结果，也就是第一条的结果是 4 秒，第二个测试的结果的是 3 秒。
