...
gethostbyname() 基于 libc 的 gethostbyname() 实现，将指定域名解析为 IP 。dnsLookup() 是另一种域名对应 IP 的查询方式，它不是基于 gethostbyname() 的。getaddrinfo() 是进行 DNS 解析，通过 DNS 信息查询对应域名的 IP 地址。

关于这三个函数，可以查询一下 C 语言中相关的资料，也可以了解一下网络相关的知识。

## 文件操作

文件操作无外乎就是对文件进行读写操作，和我们普通的 PHP 开发没什么区别，只是说调用 System 的方法是实现了协程版本的。
